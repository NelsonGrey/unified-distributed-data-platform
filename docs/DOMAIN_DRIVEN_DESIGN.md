...
api/
  ...
  admin/v1/        # AdminService (Cluster Orchestration, first slice: membership only)
deploy/
  kubernetes/      # single-node manifests
```

No `governance/`, `compatibility/`, `evidence/`, `metering/`, or `adapters/` packages exist yet — they map to the bounded contexts still marked "not started" above. `orchestration/` doesn't exist as its own package either; its one implemented slice (membership changes) lives directly in `internal/replication` (the raft config-change calls) and `internal/api` (the `AdminServer` adapter), not a dedicated package — appropriate for how little of that context is built so far, worth revisiting once `Operation`/`PlacementPlan` exist. The `internal/api` package (gRPC service adapters) sits across the native and admin API edges rather than inside a specific context, which the target layout doesn't have a named slot for either.

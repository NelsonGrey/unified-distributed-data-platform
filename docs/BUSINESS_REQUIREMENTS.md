...
| UDP-BR-004 | Operators shall provision, scale, upgrade, back up, restore, and inspect a cluster through one control plane.                                              |     Must | Scenario-based operator acceptance suite                  | Not started — no control plane exists; cluster membership is static CLI flags, not an operator workflow |
